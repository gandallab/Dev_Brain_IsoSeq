---
title: "Figure 6 - Rare Variant REVISION1"
...
df_enrichments %>% mutate(fdr = p.adjust(P, 'fdr')) %>% group_by(dx)   %>% 
    ...
    labs(x="") + ggtitle("Rare Variant Gene Enrichments",subtitle = 'Gene features from Iso-Seq, controlled for\ngene_length') + theme_bw()
```

```{r}
ggsave("output/figures/revision1/Fig6/revisedComparison/Fig6_rareVar01.pdf", width = 6, height = 6)
```

## Rare Variant Enrichment Analyses:
### 1b) Transcript, exon number, novel exon - control for coding length
...
df_enrichments.de %>% mutate(fdr = p.adjust(P, 'fdr')) %>% group_by(dx)   %>% 
    ...
    ylim(0, 10) +
    facet_wrap(dx~.,scales = 'free_y',ncol=2) + coord_flip() + geom_hline(yintercept = -log10(.05),lty=2, color='red') +
    labs(x="") + ggtitle("Rare Variant Gene Enrichments",subtitle = 'Gene features from Iso-Seq, controlled for gene_length') + theme_bw()
```

```{r}
ggsave("output/figures/revision1/Fig6/revisedComparison/Fig6_rareVar02.pdf", width = 7, height = 11)
```
...
df_enrichments.de %>% mutate(set= str_split_i(df_enrichments.de$feature, '[.]',1)) %>%  mutate(fdr = p.adjust(P, 'fdr')) %>%
  ggplot(aes(x=reorder(dx, -log10(fdr)), y=-log10(fdr), fill=set))+ geom_bar(stat='identity') + 
    ...
    labs(x="") + ggtitle("Rare Variant Gene Enrichments",subtitle = 'CP/GZ differential expression, controlled for gene_length') + theme_bw()
```

```{r}
ggsave("output/figures/revision1/Fig6/revisedComparison/Fig6_rareVar03.pdf", width = 9, height = 8.5)
```

```{r}
ggplot(df_enrichments.de %>% mutate(fdr=p.adjust(P,'fdr')), aes(y=dx, x=feature,label=ifelse(fdr<.05,signif(OR,2), ""), fill=-log10(fdr))) +
  geom_tile() + geom_text() + scale_fill_gradient(low='white',high='red', limits=c(0, 10)) + ggtitle("controlled for gene_length") +
  theme(axis.text.x = element_text(angle=-90,hjust=0))
```

```{r}
ggsave("output/figures/revision1/Fig6/revisedComparison/Fig6_rareVar04.pdf", width = 6, height = 5)
```

### 2b) DGE/DTE/DTU genes controlled for coding length
...
pheatmap(t(net), clustering_method = 'average', border_color = 'grey60',
         ...
         file = "output/figures/revision1/Fig6/revisedComparison/Fig6_rareVar05.pdf", width = 14, height = 7,
         fontsize = 10, fontsize_col = 8, main = "controlled for gene_length", breaks=seq(0, 12, 0.24)) # 0, 14, 0.28
```

### 3b) Network Modules controlled for coding length
```{r}
...
#pheatmap(t(net), clustering_method = 'average', border_color = 'grey60',
#         annotation_col = anno,color = blueWhiteRed(100)[51:100],
#         file = "output/figures/revision1/Fig6/Fig6_rareVar05_for_revision.pdf", width = 14, height = 7,
#         fontsize = 10, fontsize_col = 8, main = "controlled for gene_length|coding_length", breaks=seq(0, 12, 0.24)) # 0, 14, 0.28
```
...
```{r}
# Count number of significant (P<0.05) modules in each network
net.summary = df_enrichments.net2 %>%
  mutate(
    fdr = p.adjust(P, 'fdr'),
    net = strsplit2(feature,"[.]|_|M")[,1],
    module = strsplit2(feature,"[.]|_|M")[,3]
  ) %>%
  filter(P<0.05) %>%
  pivot_wider(id_cols = feature, names_from = dx, values_from = P) %>%
  as.data.frame()

rownames(net.summary) = net.summary$feature
net.summary = -log10(net.summary[,-1])
net.summary = net.summary %>% mutate(net = strsplit2(rownames(net.summary),"[.]|_|M")[,1])
net.summary %>% count(net)
```
...
```{r}
net2 = net
net2 = net2 %>% mutate(net = strsplit2(rownames(net2),"[.]|_|M")[,1], module = strsplit2(rownames(net2),"[.]|_|M")[,3])
```

```{r}
net2 = net2 %>% mutate(cell_class = case_when(net=="isoExpr" & module %in% c(2,15,41,4,3,11,21,12,19,23,26) ~ 'Progenitor', 
                                              net=="isoExpr" & module %in% c(16,1,30,6,20,14,38,5,8,46,34,49,45,31,47,25,17,42,9,29,35,40,53,27,48) ~ 'Exc Neuron', 
                                              net=="isoExpr" & module %in% c(39,13,7) ~ 'Inh Neuron', 
                                              net=="isoExpr" & module %in% c(10,18,22,24,28,32,33,36,37,43,44,50,51,52) ~ 'Other', 
                                              net=="isoUsage" & module %in% c(27,19) ~ 'Progenitor',
                                              net=="isoUsage" & module %in% c(2,8,14,20,15,24,10,29) ~ 'Exc Neuron',
                                              net=="isoUsage" & module %in% c(23) ~ 'Inh Neuron',
                                              net=="isoUsage" & module %in% c(4,5,6,7,9,13,16,17,18,21,22,25,26,28) ~ 'Other',
                                              net=="isoUsage" & module %in% c(3,12,11,1) ~ 'Prog/Neur',
                                              net=="geneExpr" & module %in% c(2,21,9,16,15,1) ~ 'Progenitor',
                                              net=="geneExpr" & module %in% c(5,12,6,8,10,4,11) ~ 'Exc Neuron',
                                              net=="geneExpr" & module %in% c(7,19) ~ 'Inh Neuron',
                                              net=="geneExpr" & module %in% c(14,3,13,17,18,20,22,23) ~ 'Other'))
```

```{r}
anno = data.frame(network = as.factor(strsplit2(rownames(net2),"[.]")[,1]), cell_class = as.factor(net2$cell_class))
rownames(anno) = rownames(net)

anno_colors = list(network = c(isoExpr="#69b3a2", isoUsage="purple", geneExpr="black"),
                  cell_class = c(Progenitor="goldenrod2", 'Prog/Neur'="springgreen1", 'Inh Neuron'="cyan1", Other="hotpink1", 'Exc Neuron'="mediumblue"))

heatmap_labels = t(net)
heatmap_labels[t(net) >= -log10(0.05)] <- "*"
heatmap_labels[t(net) < -log10(0.05)] <- ""

pheatmap(t(net),clustering_method = 'average', border_color = 'grey60', annotation_col = anno, annotation_colors = anno_colors, color = blueWhiteRed(100)[50:100], display_numbers = heatmap_labels)
```

```{r}
pheatmap(t(net),clustering_method = 'average', border_color = 'gray60', annotation_col = anno, annotation_colors = anno_colors, color = blueWhiteRed(100)[50:100],file = "output/figures/revision1/Fig6/revisedFinal/Fig6_rareVar05_colorBar.pdf", width = 14, height = 7,fontsize = 10, fontsize_col = 8, display_numbers = heatmap_labels, number_color = "gray30", fontsize_number = 12)
```
...
df_enrichments.singlecell %>% mutate(fdr = p.adjust(P, 'fdr'), feature = feature %>% as_factor() %>% fct_relevel(cell_type_order) %>% fct_rev()) %>% 
    ...
    labs(x="") + ggtitle("Rare Variant Gene Enrichments",subtitle = 'Single cell markers (logit), controlled for gene_length') +
    theme_bw()
```

```{r}
ggsave("output/figures/revision1/Fig6/revisedComparison/Fig6_rareVar06.pdf", width = 7, height = 9)
```

```{r}
# run subsequent cells to get SingleCellDTU bar
if (FALSE) {
  ggsave("output/figures/revision1/Fig6/revisedComparison/Fig6_rareVar06_wSingleCellDTU.pdf", width = 7, height = 9)
}
```

### 4b) Single cell markers controlled for coding length
...
df_enrichments.singlecell %>% filter(feature=="SingleCellDTU") %>% mutate(fdr = p.adjust(P, 'fdr')) %>% group_by(dx)   %>% 
    ggplot(aes(x=reorder(dx, -log10(fdr)), y=-log10(fdr)))+ geom_bar(stat='identity',fill='royalblue') + 
      ...
      labs(x="") + ggtitle("Rare Variant Gene Enrichments",subtitle = 'Single cell DTU genes (logit), controlled for gene_length') + theme_bw()
```

```{r}
ggsave("output/figures/revision1/Fig6/revisedComparison/Fig6_rareVar07.pdf", width = 4, height = 4)
```

### 5b) Single Cell DTU genes controlled for coding length
...
tableS6 %>%
  ...
  labs(
    ...
    subtitle = "Top 10 categories per rare variant dx, controlled for gene_length"
  ) +
  theme_bw() +
  facet_wrap(vars(dx), scales = 'free_y', ncol = 2)
```
...
```{r}
ggsave("output/figures/revision1/Fig6/revisedComparison/Fig6_rareVar08.pdf", width = 8.5, height = 11)
```
...
tableS6 %>% filter(OR > 1,fdr<.4) %>% group_by(dx) %>% slice_min(order_by = fdr,n = 10)  %>% 
  ...
  labs(x="") + ggtitle("Rare Variant Enrichments", subtitle = "Controlled for gene_length")
```

```{r}
ggsave("output/figures/revision1/Fig6/revisedComparison/Fig6_rareVar09.pdf", width = 9, height = 8.5)
```

```{r}
tableS6 %>% filter(OR > 1, fdr< .1, dx=="NDD.fuTADA") %>% 
  ggplot(aes(x=reorder(feature, -log10(fdr)), y=-log10(fdr),fill=class))+ geom_bar(stat='identity',)+  coord_flip() + geom_hline(yintercept = -log10(.05),lty=2, color='red') + ggtitle("NDD_fuTADA", subtitle = "Controlled for gene_length") + theme_bw() + labs(x="")
```

```{r}
ggsave("output/figures/revision1/Fig6/revisedComparison/Fig6_rareVar10.pdf", width = 8.5, height = 11)
```

```{r}
tableS6 %>% filter(OR > 1, fdr< .1, dx=="ASD.fuTADA") %>% 
 ggplot(aes(x=reorder(feature, -log10(fdr)), y=-log10(fdr),fill=class))+ geom_bar(stat='identity')+  coord_flip() + geom_hline(yintercept = -log10(.05),lty=2, color='red') + ggtitle("ASD_fuTADA", subtitle = "Controlled for gene_length") + theme_bw() + labs(x="")
```

```{r}
ggsave("output/figures/revision1/Fig6/revisedComparison/Fig6_rareVar11.pdf", width = 7, height = 6)
```
...
tableS6 %>% filter(OR > 1) %>% group_by(class) %>% slice_min(order_by = fdr,n = 10)  %>% 
    ...
    labs(x="") + ggtitle("Rare Variant Enrichments", subtitle = "Net, controlled for gene_length")
```

```{r}
ggsave("output/figures/revision1/Fig6/revisedComparison/Fig6_rareVar12.pdf", width = 9, height = 8.5)
```



## Combine All controlled for coding length
